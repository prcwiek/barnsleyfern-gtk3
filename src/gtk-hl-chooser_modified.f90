...
!*
! File Choosers
module gtk_hl_chooser_modified
...
contains
  ...
  !+
  function hl_gtk_file_chooser_new_modified(chooser_info, cdir, directory, create, &
    ...
    ! Modification 2022-11-27:
    ! Here is a modification which allow to change a name of left button
    ! Two below line moved behind Section 'Decode the action
    ! Added if for action
    ! *******************************************************************
    if (action == GTK_FILE_CHOOSER_ACTION_OPEN) then
        junk = gtk_dialog_add_button(dialog, "_Open"//C_NULL_CHAR, GTK_RESPONSE_APPLY)
    else 
        junk = gtk_dialog_add_button(dialog, "_Save"//C_NULL_CHAR, GTK_RESPONSE_APPLY)
    end if
  ...
  end function hl_gtk_file_chooser_new_modified

  !+
  function hl_gtk_file_chooser_show_modified(files, cdir, directory, create, &
       & multiple, allow_uri, show_hidden, confirm_overwrite, title, &
       & initial_dir, current, initial_file, filter, filter_name, parent, &
    ...
    dialog =  hl_gtk_file_chooser_new_modified(chooser_info, cdir, directory, create, &
         & multiple, allow_uri, show_hidden, confirm_overwrite, title, &
         & initial_dir, current, initial_file, filter, filter_name, parent, &
  ...
  end function hl_gtk_file_chooser_show_modified
...
end module gtk_hl_chooser_modified
